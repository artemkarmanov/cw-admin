...
export class SessionDialogComponent implements OnInit, OnDestroy {
  ...
  public data$$: BehaviorSubject<ISession | null> =
    new BehaviorSubject<ISession | null>(null);
  private data$: Observable<ISession> = this.data$$
    .asObservable()
    .pipe(filter(Boolean));
  ...
  constructor(private modal: DialogRef) {
    const controls = {
      ...
      sessionDurationMins: new FormControl(DEFAULT_SESSION_DURATION, [
        Validators.required,
        Validators.min(MINIMUM_SESSION_DURATION),
      ]),
      audioDetailsOverride: new FormControl({ value: '', disabled: true }),
      audioDetailsOverrideCheck: new FormControl({ value: true }),
      captionDispOverride: new FormControl({ value: '', disabled: true }),
      captionDispOverrideCheck: new FormControl({ value: true }),
    ...
    this.data$
      .pipe(
        ...
        tap((session: ISession) => {
          ...
          const ISOTimeA = DateTime.fromSeconds(session.startEpoch)
            .toISOTime()
            .split(':');
          const startTime = {
            hour: +ISOTimeA[0],
          ...
          this.form
            .get('sessionDurationMins')
            ?.setValue(session.sessionDurationMins);
          this.form
            .get('audioDetailsOverride')
            ?.setValue(session.audioDetailsOverride);
          this.form
            .get('captionDispOverride')
            ?.setValue(session.captionDispOverride);
          this.form.get('nonBilled')?.setValue(session.nonBilled);
          this.form
            .get('respeakerRateOverride')
            ?.setValue(session.respeakerRateOverride);
  ...
  save() {
    if (this.form.valid) {
      ...
      const transformedValue =
        ('0' + new Date().getHours()).slice(-2) + ':' + new Date().getMinutes();
      const startEpoch = DateTime.fromISO(
        [startDate, transformedValue].join('T')
      ).toSeconds();
      ...
      this.modal.close({
        ...
        ...(respeakerRateOverride &&
          parseFloat(respeakerRateOverride) && {
            respeakerRateOverride: parseFloat(respeakerRateOverride),
          }),
        ...(nonBilled !== undefined && { nonBilled: nonBilled ? 1 : 0 }),
        ...(allowOverrun !== undefined && {
          allowOverrun: allowOverrun ? 1 : 0,
        }),
      });
    }
  }
